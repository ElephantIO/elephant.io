...
class Polling extends Transport
{
    ...
    {
        $hash = sha1(uniqid((string) mt_rand(), true), true);
     ...
     *
     * @param string $uri
     * @param array<string, mixed> $headers Key-value pairs
     ...
     * Get response status.
     *
     * @return array<int, mixed>|null Index 0 is HTTP version, index 1 is status code, and index 2 is status message
     */
    ...
    protected function decodeChunked($str)
    {
        for ($res = ''; !empty($str); $str = trim($str)) {
            if (false !== ($pos = strpos($str, StreamInterface::EOL))) {
                $len = (int) hexdec(substr($str, 0, $pos));
    ...
    public function recv($timeout = 0, $parameters = [])
    {
        ...
        if ($this->getStatusCode() === $code) {
            $body = $this->getBody();
            if (is_string($body)) {
                return new Decoder($body, $this->sio->getOptions()->version,
                    $this->getHeader('Content-Type') === static::MIMETYPE_OCTET_STREAM);
            } elseif (is_array($body)) {
                if ($encoded = json_encode($body)) {
                    return $encoded;
                }
            } else {
